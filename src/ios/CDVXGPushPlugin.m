...
- (void)pluginInitialize {
  ...
  uint32_t accessId =
      [[[[NSBundle mainBundle] objectForInfoDictionaryKey:@"XGPushMeta"]
          valueForKey:@"AccessID"] intValue];
  NSString *accessKey = [[[NSBundle mainBundle]
      objectForInfoDictionaryKey:@"XGPushMeta"] valueForKey:@"AccessKey"];
...
- (void)startApp:(uint32_t)assessId key:(NSString *)accessKey {
  NSLog(@"[XGPushPlugin] starting with access id: %u, access key: %@", assessId,
        accessKey);
...
- (void)sendMessage:(NSString *)type data:(NSDictionary *)dict;
{
  if (self.callbackId != nil) {
    NSMutableDictionary *newDict =
        [NSMutableDictionary dictionaryWithObjectsAndKeys:type, @"type", nil];
...
- (void)registerPush:(CDVInvokedUrlCommand *)command {
  ...
  NSLog(@"[XGPushPlugin] registerPush: account = %@, token = %@", account,
...
- (void)getLaunchInfo:(CDVInvokedUrlCommand *)command {
  ...
  if (_luanchOptions) {
    result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                           messageAsDictionary:_luanchOptions];
  } else {
    result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
  }
...
- (void)xgPushDidFinishStart:(BOOL)isSuccess error:(NSError *)error {
  NSLog(@"%s, result %@, error %@", __FUNCTION__, isSuccess ? @"OK" : @"NO",
        error);
...
- (void)xgPushDidRegisteredDeviceToken:(NSString *)deviceToken
                                 error:(NSError *)error {
  NSLog(@"%s, result %@, error %@", __FUNCTION__, error ? @"NO" : @"OK", error);
}
// iOS 10 新增 API
...
- (void)xgPushUserNotificationCenter:(UNUserNotificationCenter *)center
  ...
  NSMutableDictionary *customContent =
      [NSMutableDictionary dictionaryWithCapacity:userInfo.count - 2];
...
/**
 前台收到通知
 */
- (void)xgPushDidReceiveRemoteNotification:(id)notification
                     withCompletionHandler:
  ...
  if ([notification isKindOfClass:[NSDictionary class]]) {
    ...
    completionHandler(UNNotificationPresentationOptionBadge |
                      UNNotificationPresentationOptionSound |
                      UNNotificationPresentationOptionAlert);
    UNNotification *notify = (UNNotification *)notification;
    UNNotificationContent *content = notify.request.content;
    NSDictionary *userInfo = content.userInfo;
    NSMutableDictionary *customContent =
        [NSMutableDictionary dictionaryWithCapacity:userInfo.count - 2];
    for (NSString *key in userInfo) {
...
- (void)xgPushDidBindWithIdentifiers:(NSArray<NSString *> *)identifiers
                                type:(XGPushTokenBindType)type
                               error:(NSError *)error {
...
- (void)xgPushDidUnbindWithIdentifiers:(NSArray<NSString *> *)identifiers
                                  type:(XGPushTokenBindType)type
                                 error:(NSError *)error {
  NSLog(@"%s, id is %@, error %@", __FUNCTION__, identifiers, error);
  [self sendCallback:error];
...
- (void)xgPushDidUpdatedBindedIdentifiers:(NSArray<NSString *> *)identifiers
